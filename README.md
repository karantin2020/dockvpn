...
```bash
CID=$(docker run -d --privileged -p 1194:1194/udp -p 443:443/tcp node3030/dockvpn)
docker run -t -i -p 8080:8080 --volumes-from $CID node3030/dockvpn serveconfig
```
...
When the `node3030/dockvpn` image is started, it generates:
...
Conveniently, `node3030/dockvpn` comes with a script called `serveconfig`,
